import enum, logging, os, jwt, json, requests, constants
import pandas as pd
...
] = f"mysql://{DB_USER}:{DB_PASS}@{DB_IP}:{DB_PORT}/{DB_SCHEMA}"
...
swaggerui_blueprint = get_swaggerui_blueprint(
    SWAGGER_URL,
    API_URL,
    config={"app_name": "WYS API - Prices Service"},  # Swagger UI config overrides
)
"""Swagger blueprint"""
...
class PriceModule(db.Model):
    ...
    PriceModule 
    ---
    it provides SQl methods to connect with price_module table on DB
    
    Attributes
    ---
    id: Id primary key
    name: Space name (Same name that is in spaces table)
    ...
    values = db.relationship(
        "PriceValue", backref="price_module", cascade="all, delete, delete-orphan"
    )
...
class PriceCategory(db.Model):
    ...
    PriceCategory
    ---
    It provides SQl methods to connect with price_category table on DB
    
    Attributes
    ---------
    id: Id primary key
    code: PriceCategory Name Code

        Can be CALIDAD_TERMINACIONES, MOBILIARIO,
        CALIDAD_ACUSTICA, CONFORT_CLIMATICO,
        VELOCIDAD_RED, CONTROL_ILUMINACION,
        SEGURIDAD, TECNOLOGIA, ELECTRODOMESTICOS
                                    
    name: PriceCategory Name that will be displayed in frontend (In spanish)
    type: It could be A or B
    parent_category_id: Relation with PriceCategory id
    """
    ...
    """It contains all categories related to the parent category"""
    values = db.relationship(
        "PriceValue", backref="price_category", cascade="all, delete, delete-orphan"
    )
    """It contains records related on price_value table"""
...
class PriceGen(db.Model):
    ...
    PriceGen 
    ---
    it provides SQl methods to connect with price_gen table on DB
    
    Attributes
    ---
    id:  Id primary key
    project_id: Project ID that it's required to save this configurations
    ...
    relations = db.relationship(
        "PriceGenHasPriceValue",
        backref=db.backref("price_gen_has_price_value", remote_side=[id]),
        cascade="all, delete, delete-orphan",
    )
    """it gets all records related in `PriceGenHasPriceValue()`"""

    def to_dict(self, full=True):
        """
        Convert to dictionary if it's required
        """
        obj_dict = {"project_id": self.project_id, "value": self.value, "m2": self.m2}
        if full:
            obj_dict["price_value_saved"] = [
...
class PriceDesign(db.Model):
    ...
    Class PriceDesign 
    ---
    it provides SQl methods to connect with price_design table on DB
    ...
    country_id = db.Column(
        db.Integer, db.ForeignKey("price_country.id"), nullable=False
    )
...
class PriceGenHasPriceValue(db.Model):
    ...
    PriceGenHasPriceValue 
    ---
    it provides SQl methods to connect with price_has_price_value table on DB
    
    Attributes
    ---
    id: Id primary key
    price_gen_id: ID related to the project's value generated in PriceGen
    price_value_id: ID related to the item (category, module, country) in PriceValue
    price_value_option_selected: option selected by the user: low, high, medium

    """

    id = db.Column(db.Integer, primary_key=True)
    price_gen_id = db.Column(db.Integer, db.ForeignKey("price_gen.id"), nullable=False)
    price_value_id = db.Column(
        db.Integer, db.ForeignKey("price_value.id"), nullable=False
    )
    price_value_option_selected = db.Column(db.String(45), nullable=False)
    prices_value = db.relationship(
        "PriceValue",
        backref=db.backref("price_value", remote_side=[price_value_id]),
        cascade="all, delete, delete-orphan",
        single_parent=True,
    )
    """It has all `prices_value` value records, using `PriceValue()` """
...
class PriceCountry(db.Model):
    ...
    PriceCountry
    ---
    it provides SQl methods to connect with price_country table on DB
    ...
    values = db.relationship(
        "PriceValue", backref="price_country", cascade="all, delete, delete-orphan"
    )
...
class PriceValue(db.Model):
    ...
    PriceValue 
    ---
    it provides SQl methods to connect with price_value table on DB
    
    Attributes
    ---
    id: Id primary key
    low: value if it's selected "low option"
    medium: value if it's selected "medium option"
    high: value if it's selected "high option"
    module_id: id of the module related in PriceModule
    country_id: id of the country related in PriceCountry
    category_id: id of the category related in PriceCategory
    ...
    category_id = db.Column(
        db.Integer, db.ForeignKey("price_category.id"), nullable=False
    )
    ...
    """it gets the country name from `PriceCountry()`"""
...
def token_required(f):
    ...
    def decorator(*args, **kwargs):
        ...
        try:
            data = jwt.decode(
                token, app.config["SECRET_KEY"], algorithms=["RS256"], audience="1"
            )
...
def get_project_weeks(m2, token):
    """from times module, it gets the data related"""
    try:
...
def get_workspace_by_project_id(project_id, token):
    """Returns all the workspace related to a projectId"""
    headers = {"Authorization": token}
...
def spec():
    ...
    swag["tags"] = [{"name": "Prices", "description": "Methods to configure Prices"}]
    return jsonify(swag)
...
def upload_design_prices():
    ...
    try:
        ...
        if not (
            filename_split[-1] == constants.VALID_EXTENSIONS_XLS
            or filename_split[-1] == constants.VALID_EXTENSIONS_XLSX
        ):
        ...
        for country_name in sheets:
            try:
                ...
                country: PriceCountry = PriceCountry.query.filter(
                    PriceCountry.name == country_name.upper()
                ).first()
            ...
            try:
                qr = PriceDesign.query.filter(
                    PriceDesign.country_id == country_id
                ).first()
            except Exception as exp:
...
def upload_prices():
    ...
    for country_name in sheets:
        ...
        for row in sheets[country_name].iterrows():
            ...
            category_name = (
                row[1][constants.ROW_MODULO]
                if is_base
                else row[1][constants.ROW_PARAMETRO]
            )
            ...
            if not last_category_name is None and last_category_name != category_name:
                ...
                try:
                    if is_base:
                        value = (
                            PriceValue.query.filter(PriceValue.country_id == country_id)
                            .filter(PriceValue.category_id == last_category.id)
                            .first()
                        )
                    else:
                        value = (
                            PriceValue.query.filter(
                                PriceValue.module_id == modules_hash[module_name].id
                            )
            ...
            if category_name not in category_hash:
                category: PriceCategory = PriceCategory.query.filter(
                    PriceCategory.name == category_name
                ).first()
            ...
            if have_subcat:
                ...
                if subcategory_code not in subcategory_hash:
                    subcategory: PriceCategory = (
                        PriceCategory.query.filter(
                            PriceCategory.name == subcategory_name
                        )
                    ...
                    if subcategory is None:
                        try:
                            ...
                            subcategory.code = (
                                subcategory_code if not is_base else "BASE"
                            )
            ...
            try:
                low: float = (
                    row[1][constants.ROW_BAJO]
                    if not pd.isna(row[1][constants.ROW_BAJO])
                    else 0
                )
                medium: float = (
                    row[1][constants.ROW_MEDIO]
                    if not pd.isna(row[1][constants.ROW_MEDIO])
                    else 0
                )
                high: float = (
                    row[1][constants.ROW_ALTO]
                    if not pd.isna(row[1][constants.ROW_ALTO])
                    else 0
                )

            except Exception as exp:
                msg = (
                    f"Error reading rows: {constants.ROW_BAJO}, "
            ...
            subcategory_id = (
                subcategory_hash[subcategory_code].id
                if have_subcat
                else category_hash[category_name].id
            )

            module = modules_hash[module_name] if not is_base else None
            subcategory = (
                subcategory_hash[subcategory_code]
                if have_subcat
                else category_hash[category_name]
            )
            ...
            try:
                if is_base:
                    value = (
                        PriceValue.query.filter(PriceValue.country_id == country_id)
                        .filter(PriceValue.category_id == subcategory_id)
                        .first()
                    )
                else:
                    value = (
                        PriceValue.query.filter(PriceValue.module_id == module_id)
                        .filter(PriceValue.country_id == country_id)
                        .filter(PriceValue.category_id == subcategory_id)
                        .first()
                    )
...
def get_categories():
    ...
    try:
        ...
        categories: list = PriceCategory.query.filter(
            PriceCategory.parent_category_id == None
        ).all()
...
def save_prices():
    ...
    # Check JSON Input
    params = {"categories", "country", "project_id", "value", "m2", "workspaces"}

    for param in params:
        if param not in request.json:
            logging.error(f"{param} not in body")
            return jsonify({"message": f"{param} not in body"}), HTTPStatus.BAD_REQUEST

    try:
        token = request.headers.get("Authorization", None)
        headers = {"Authorization": token}
        resp = requests.get(
            f"{PROJECTS_URL}{PROJECTS_MODULE_API}" f'/{request.json["project_id"]}',
            headers=headers,
        )
    ...
    # new id
    price_gen: PriceGen = PriceGen.query.filter(
        PriceGen.project_id == request.json["project_id"]
    ).first()
    ...
    try:
        ...
        if project["m2_gen_id"] is None:
            ...
            token = request.headers.get("Authorization", None)
            m2_generated = get_workspace_by_project_id(
                request.json["project_id"], token
            )

            if m2_generated is None:
                logging.warning(
                    f'No data founded in m2 generated: project#{request.json["project_id"]}'
                )
                price_gen.m2 = request.json["m2"]
            else:
    ...
    project = update_project_by_id(
        request.json["project_id"], {"price_gen_id": price_gen_id}, token
    )
    if project is None:
    ...
    try:
        ...
        logging.error(exp)
        return {"message": f"{exp}"}, HTTPStatus.BAD_REQUEST
    ...
    while i < len(workspaces):
        ...
        if price_module is None:
            ...
            for category in categories:
                ...
                prices = (
                    PriceValue.query.filter(PriceValue.country_id == country.id)
                    .filter(PriceValue.module_id == module_id)
                    .filter(PriceValue.category_id == category["id"])
                    .first()
                )
                if prices is None:
                    logging.warning(
                    ...
                    pghpv = (
                        PriceGenHasPriceValue.query.filter(
                            PriceGenHasPriceValue.price_gen_id == price_gen_id
                        )
                        .filter(PriceGenHasPriceValue.price_value_id == prices.id)
                        .first()
                    )
...
def update_project_by_id(project_id, data, token):
    """For update price_gen_id in project table"""
    headers = {"Authorization": token}
...
def get_design_cost(country_id, m2):
    ...
    try:
        pd = PriceDesign.query.filter(PriceDesign.country_id == country_id).first()
...
def get_estimated_price():
    ...
    # Check JSON Input
    params = {"categories", "workspaces", "country", "m2"}
    ...
    country_name = request.json["country"]
    country: PriceCountry = PriceCountry.query.filter(
        PriceCountry.name == country_name
    ).first()
    if country is None:
        return f"{country_name} is a invalid country"
    ...
    while i < len(workspaces):
        ...
        if price_module is None:
            ...
            prices = PriceValue.query.filter(
                PriceValue.country_id == country.id
    ...
    base_prices = PriceValue.query.filter(PriceValue.country_id == country.id).filter(
        PriceValue.module_id == None
    )
    ...
    for category in categories:
        ...
        if category["code"] != "BASE":
            for _space in workspaces:
                ...
                if space_id in space_category_prices:
                    if cat_id in space_category_prices[space_id]:
                        final_value += (
                            space_category_prices[space_id][cat_id][cat_resp]
                        ) * _space["quantity"]
                    else:
            ...
            if cat_id in space_category_prices[-1]:
                if calc_type == "m2":
                    final_value += space_category_prices[-1][cat_id][cat_resp] * (
                        m2 / div_factor
                    )
                elif calc_type == "weeks":
                    final_value += space_category_prices[-1][cat_id][cat_resp] * weeks
                else:
                    final_value += space_category_prices[-1][cat_id][cat_resp]
            else:
                logging.warning(f"Not valid cat_id: {category['id']}")
    ...
    price_design: PriceDesign = PriceDesign.query.filter(
        PriceDesign.country_id == country.id
    ).first()
...
def get_estimated_price_detail():
    ...
    for category in categories:
        ...
        cat_obj = PriceCategory.query.filter(
            PriceCategory.name == category["name"]
        ).first()
        ...
        if category["code"] != "BASE":
                        ...
                        if cat_subcategories:
                            for subcat in category["subcategories"]:
                                if subcat["id"] in space_category_prices[space_id]:
                                    subcat["value"] += (
                                        space_category_prices[space_id][subcat["id"]][
                                            cat_resp
                                        ]
                                    ) * _space["quantity"]
                                else:
                                    logging.warning(
                                        f"Not valid subcat_id: {subcat['id']}"
                                    )
                    else:
                        logging.warning(f"Not valid cat_id: {category['id']}")
                else:
            ...
            if cat_id in space_category_prices[-1]:
                if calc_type == "m2":
                    cat_value = space_category_prices[-1][cat_id][cat_resp] * (
                        m2 / div_factor
                    )
                    category["value"] = cat_value
                    final_value += cat_value
                        ...
                        for subcat in category["subcategories"]:
                            if subcat["id"] in space_category_prices[-1]:
                                subcat["value"] += space_category_prices[-1][
                                    subcat["id"]
                                ][cat_resp] * (m2 / div_factor)
                            else:
                                logging.warning(f"Not valid subcat_id: {subcat['id']}")
                elif calc_type == "weeks":
                    cat_value = space_category_prices[-1][cat_id][cat_resp] * weeks
                    category["value"] = cat_value
                    final_value += cat_value
                    if cat_subcategories:
                        for subcat in category["subcategories"]:
                            if subcat["id"] in space_category_prices[-1]:
                                subcat["value"] += (
                                    space_category_prices[-1][subcat["id"]][cat_resp]
                                    * weeks
                                )
                            else:
                                logging.warning(f"Not valid subcat_id: {subcat['id']}")
    ...
    pd = PriceDesign.query.filter(PriceDesign.country_id == country.id).first()
